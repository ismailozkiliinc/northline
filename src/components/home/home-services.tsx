...
export function HomeServices() {
  ...
  return (
    <section id="services" className="border-t border-white/8 bg-transparent py-16 md:py-24">
      <div className="container-page">
        <div className="max-w-xl">
          ...
          <p className="mt-4 max-w-lg text-sm leading-relaxed text-[#98A2B3] md:text-base">{t("subtitle")}</p>
        </div>
        ...
        <ul className="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {categories.map((category, index) => {
            ...
            return (
              ...
              >
                <article className="flex h-full flex-col overflow-hidden rounded-[1.5rem] border border-white/8 bg-transparent shadow-[0_16px_40px_-24px_rgba(15,23,42,0.12)]">
                  <div className="relative h-[200px] overflow-hidden md:h-[220px]">{Visual()}</div>
                  <div className="flex flex-1 flex-col p-6">
                    <div className="flex items-center gap-2 text-[#6366f1]">
                      <Icon className="h-4 w-4" aria-hidden />
                      <span className="font-mono text-[11px] tracking-[0.16em] text-[#98A2B3] uppercase">
                        {String(index + 1).padStart(2, "0")}
                      </span>
                    </div>
                    <h3 className="mt-3 font-display text-xl font-bold tracking-tight text-[#F7F9FC]">{category.title}</h3>
                    <p className="mt-2 line-clamp-2 text-sm leading-relaxed text-[#98A2B3]">{category.description}</p>
                  </div>
                </article>
              </motion.li>
            );
